...
All course and study notebooks live in `fundamentals/codes/`. Larger applied projects stay under `projects/`.
...
Start Jupyter from the repository root and open any notebook in `fundamentals/codes/`. Many notebooks use built-in scikit-learn datasets, but `One_Hot_Encoding.ipynb` downloads the Adult dataset from OpenML and `KNN_regression.ipynb` plus `SVM.ipynb` download the California housing dataset on first run.
